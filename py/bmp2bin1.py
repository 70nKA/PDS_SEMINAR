def bmp_to_hex_per_byte(filepath, output_filepath):
    ...
    # Save each byte in hexadecimal format to a text file, each on a new line
    with open(output_filepath, 'w') as output_file:
        for byte in binary_pixel_data:
            hex_value = f"{byte:02x}"
            output_file.write(f"{hex_value}\n")

    print(f"Hexadecimal pixel data has been saved to {output_filepath}")
    return binary_pixel_data

if __name__ == "__main__":
    bmp_to_hex_per_byte("lena_256.bmp", "lena_hex2.txt")
